...
public class Bug {
	...
	public void go() throws GameActionException {
		if (Clock.getRoundNum() > 400) {
			Robot[] enemies = rc.senseNearbyGameObjects(Robot.class, 8, rc.getTeam().opponent());
			Robot[] friends = rc.senseNearbyGameObjects(Robot.class, 2, rc.getTeam());
			if (enemies.length > 0 && friends.length < 1) return;
		}
		...
		if (dir.ordinal() == back ||
			...
			//depth = 3;
			toggleDirection();
			if (turnDir == 1) stuck = rc.getLocation(); //BROADCAST STUCK SPOTS!!!
			dir = turn(dir2target);
		}
