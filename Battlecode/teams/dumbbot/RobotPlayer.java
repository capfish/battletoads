            ...
            try {
                if (rc.getType() == RobotType.HQ) {
                    ...
                    if (rc.isActive()) {
                        ...
                        if(Clock.getRoundNum() < 10000){
                            if (Math.random() < .2) {// && rc.getLocation().distanceSquaredTo(rc.senseHQLocation()) < 16) {
                                // Lay a mine 
                                if(rc.senseMine(rc.getLocation())==null) rc.layMine();
                            } else if (nearbyEncampments.length != 0) {
                                if (rc.senseEncampmentSquare(rc.getLocation())) rc.captureEncampment(RobotType.ARTILLERY);
                                else {
