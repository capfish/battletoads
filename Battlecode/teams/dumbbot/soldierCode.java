...
//check if mines are present before making mines!!!!!
	...
	private static void travelMode(RobotController rc) throws GameActionException
	{
		...
		if (!rc.canMove(dir)) dir = randomDir(rc);
                MapLocation squareAhead = myLoc.add(dir);
		Team t = rc.senseMine(squareAhead);
		if ((t == Team.NEUTRAL || t == RobotPlayer.enemyTeam) && !(msg.receive().equals("x: "+ squareAhead.x + " y: " + squareAhead.y))){
                    msg.sendLoc(squareAhead);
                    rc.defuseMine(squareAhead);
                } else if (msg.receive().equals("x: "+ squareAhead.x + " y: " + squareAhead.y)){
