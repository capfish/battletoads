...
public class soldierCode {
    ...
    private static boolean colonizeMode(RobotController rc) throws GameActionException {
    	...
    	/*int minDist = 10000;
          ...
          ppp    	for (MapLocation encamp: encamps) {
    ...
    private static boolean capture(RobotController rc) throws GameActionException{
        ...
        //        else if (myLoc.distanceSquaredTo(RobotPlayer.myHQ) < 64 ||
	...
	/*--------------MINE CODE--------------------*/
        ...
        private static boolean mineMode(RobotController rc) throws GameActionException {
            ...
            if (rc.hasUpgrade(Upgrade.PICKAXE))
                {
                    if (myLoc.distanceSquaredTo(RobotPlayer.myHQ) < 25 && pickaxeMineField(rc))
                        {
                            rc.layMine();
                            return true;
                        }
            ...
            else if (myLoc.distanceSquaredTo(RobotPlayer.myHQ) < 25 && sparseMineField(rc) && !rc.senseEncampmentSquare(myLoc))
                {
                    rc.layMine();
                    return true;
                }
